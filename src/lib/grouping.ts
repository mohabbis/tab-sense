...
import { humanizeSlug, parseUrl, registrableDomain } from "@/lib/url";
...
function looksLikeId(value: string): boolean {
  return (
    ...
    /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f-]{16}$/i.test(value)
  );
}

export function extractSignals(tab: Tab): TabSignal {
  const parsed = parseUrl(tab.url);
  const host = parsed?.hostname.replace(/^www\./, "").toLowerCase() ?? "";
  const registrable = host ? registrableDomain(host) : "";
  const pathParts = parsed
    ? parsed.pathname.split("/").filter(Boolean).map((part) => decodeURIComponent(part))
    : [];
  ...
  if (host === "linear.app" && pathParts[0]) {
    ...
    const project = pathParts[1] === "project" ? pathParts[2] : undefined;
    if (project) {
      keys.push({
  ...
  if (host === "vercel.com" && pathParts[0] && pathParts[1]) {
    const skip = new Set(["docs", "login", "signup", "blog", "pricing", "dashboard"]);
  ...
  if (host === "figma.com" || host === "www.figma.com") {
    const name = pathParts[0] === "design" || pathParts[0] === "file" ? pathParts[2] : undefined;
...
function orgTokensOf(signals: TabSignal[]): Set<string> {
...
function contentTokens(tokens: Set<string>, orgs: Set<string>): Set<string> {
  const result = new Set<string>();
  for (const token of tokens) {
...
function distinctiveSlugs(signals: TabSignal[]): Set<string> {
...
function sharesSlug(a: TabSignal[], b: TabSignal[]): boolean {
...
function colorFor(id: string): GroupColor {
  let hash = 0;
...
function strongestKey(signals: TabSignal[]): SignalKey | undefined {
  return signals
    .flatMap((signal) => signal.keys)
    .sort((a, b) => b.weight - a.weight)[0];
}

function nameFromSignals(signals: TabSignal[], kind: GroupKind): string {
  if (kind === "topic") {
    const topics = [...new Set(signals.flatMap((signal) => signal.topics))];
    const preferred =
      topics.find((topic) => topic !== "mdn" && topic !== "web") ?? topics[0];
  ...
  if (strong && strong.weight >= 6) {
    if (strong.key.startsWith("repo:")) {
      ...
      return repo ? `${humanizeSlug(org)} · ${humanizeSlug(repo)}` : humanizeSlug(strong.label);
    }
    if (strong.key.startsWith("vercel:")) {
      const [team, project] = strong.label.split("/");
      return project ? `${humanizeSlug(team)} · ${humanizeSlug(project)}` : humanizeSlug(strong.label);
    }
...
function reasonFor(kind: GroupKind, signals: TabSignal[]): string {
  const strong = strongestKey(signals);
...
function kindFor(signals: TabSignal[], mode: GroupingMode): GroupKind {
  const strong = strongestKey(signals);
...
function sortGroups(groups: TabGroup[]): TabGroup[] {
  const rank: Record<GroupKind, number> = {
    project: 0,
    topic: 1,
    tool: 2,
    loose: 3,
...
export function groupTabs(
  ...
  function ensureGroup(group: TabGroup) {
  ...
  const keyIndex = new Map<string, { weight: number; tabIds: string[] }>();
  ...
  const takeCluster = (ids: string[], kind: GroupKind, prefix: string) => {
    const clusterSignals = ids.map((id) => signals.get(id)!);
  ...
  if (shouldUseProjects || mode === "domain") {
    for (const cluster of uf.clusters()) {
      ...
      const isProject =
        mode === "domain"
          ? cluster.length >= 1
          : Boolean(strong && strong.weight >= 7);
      if (isProject) {
  ...
  for (const tabId of leftovers) {
    ...
    for (const group of attachable) {
      ...
      const orgs = new Set([...orgTokensOf([signal]), ...orgTokensOf(groupSignals)]);
      ...
      const orgMatch = signal.keys.some((key) =>
        groupSignals.some((item) => item.keys.some((other) => other.key === key.key && key.key.startsWith("org:"))),
      );
      const score = jaccard(tabTokens, groupTokens);
      const slugHit = [...distinctiveSlugs(groupSignals)].some((slug) => tabTokens.has(slug));
      const qualifies =
        slugHit ||
        score >= (mode === "project" ? 0.28 : 0.18) ||
        (orgMatch && score >= 0.12);
  ...
  const result = [...groups.values()]
    ...
    .filter((group) => group.tabIds.length > 0 || customGroups.some((custom) => custom.id === group.id))
    .map((group) => ({
      ...group,
...
export function nextGroupColor(existing: GroupColor[]): GroupColor {
